...
class ReportsRepository {
	...
	public function DashboardReport($start=null,$end=null)
	{
		...
		$where = $where." and calldate between '".$start." 00:00:00' and '".$end." 23:59:59'";
		
		$Result = DB::connection('mysql3')->table('cdr')->select(DB::raw("DATE_FORMAT(calldate, '%Y-%m-%d %H:00') Createdhour, count(*) as Total, IFNULL(sum(case when dst in ($userExtention) then 1 end),0) as Inbound, IFNULL(sum(case when src in ($userExtention) AND Length(dst)>4 then 1 end),0) as Outbound, sum(case when billsec>0 then 1 else 0 end) as Completed, sum(case when billsec=0 then 1 else 0 end) as Missed, sum(billsec) as Duration"))
		->whereRaw($where)
		...
		$query = "select sum(incall) as incall, sum(answer) as answer, sum(abandon) as abandon, sum(holdtime) as holdtime FROM queuestats where queue IN ('4001', '4002') and created_at between '".$start."' and '".$end."'";
		...
		$query = "select count(*) as answer from queue_log where verb in ('CONNECT') and created between '".$start."' and '".$end."' and call_id not in ( select call_id from queue_log where verb in ('COMPLETEAGENT', 'COMPLETECALLER')  and queue IN ('4001', '4002')  and created between '".$start."' and '".$end."')";
		$Result = DB::connection('mysql2')->select($query);
		foreach($Result as $row)
		{
		...
		$query = "select ROUND(sum(data1)) AS waittime from queue_log where verb='CONNECT' and queue IN ('4001', '4002') and created between '".$start."' and '".$end."'";
		$Result = DB::connection('mysql2')->select($query);
		foreach($Result as $row)
		{
		...
		$query = "select ROUND(sum(data2)) AS talktime from queue_log where verb='COMPLETEAGENT' and queue IN ('4001', '4002') and created between '".$start."' and '".$end."'";
		$Result = DB::connection('mysql2')->select($query);
		foreach($Result as $row)
		{
		...
		$query = "select sum(data2) AS totaltime from queue_log where verb in ('COMPLETECALLER', 'COMPLETEAGENT') and queue IN ('4001', '4002') and created between '".$start."' and '".$end."'";
		$Result = DB::connection('mysql2')->select($query);
		foreach($Result as $row)
		{
		...
		$query = "select count(*) as waiting from queue_log where verb in ('ENTERQUEUE') and created between '".$start."' and '".$end."' and call_id not in ( select call_id from queue_log where verb in ('CONNECT', 'ABANDON')  and queue IN ('4001', '4002')  and created between '".$start."' and '".$end."')";
		$Result = DB::connection('mysql2')->select($query);
		foreach($Result as $row)
		{
...
//                ->get();
    ...
    public function distribution($req)
    {
        ...
        $query = "select queue, count(distinct call_id) as received,
                  ...
                  and queue in (\"" . $json['available_queue'] . "\")
                  /*and agent in (\"". $json['agents']."\")*/
                  and created between '" . $start . "' and '" . $end . "' group by queue";
...
//        }
        ...
        $query = "select DATE_FORMAT(created,'%M %Y') as month,queue, count(distinct call_id) as received,
                  ...
                  and created between '" . $start . "' and '" . $end . "' group by month order by id";
...
//        }
        ...
        $query = "select Week(created) AS week, 
                  ...
                  /*and agent in (\"". $json['agents']."\")*/
                  and verb IN ('ABANDON', 'CONNECT','ENTERQUEUE')
                  and created between '" . $start . "' and '" . $end . "' " . $groupby . " order by id";
...
//            }
        ...
        $query = "select Hour(created) AS hour, 
                  ...
                  and verb IN ('ABANDON', 'CONNECT','ENTERQUEUE')
                  and DATE_FORMAT(created, '%H:%i') between '" . $starthr . "' and '" . $endhr . "'
                  and created between '" . $start . "' and '" . $end . "' " . $groupby . " order by hour";
...
//            }
        ...
        $query = "select Dayname(created) AS day, 
                  ...
                  /*and agent in (\"". $json['agents']."\")*/
                  and verb IN ('ABANDON', 'CONNECT','ENTERQUEUE')
                  and created between '" . $start . "' and '" . $end . "' " . $groupby . " order by DAYOFWEEK(created)";
